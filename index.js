...
async function setChoice(element){
    ...
    let housechoice = choice[Math.trunc(Math.random() * 5)]; //make a random house selection
    ...
    if(element.id === housechoice)
    {
        ...
        switch (element.id) {
            ...
            case "lizard":
                if(housechoice === "spock" || housechoice === "paper"){
                    document.querySelector(".result").innerHTML = "YOU WIN";
                    win();
                }
                else{
...
function addImage(id, player, name)
{
    ...
    switch (id) {
        ...
        case "rock":
            choice = ["images/icon-rock.svg", "rock-pic"];
            break;

        case "lizard":
            choice = ["images/icon-lizard.svg", "lizard-pic"];
            break;

        case "spock":
            choice = ["images/icon-spock.svg", "spock-pic"];
...
function reset()
{
    ...
    document.querySelector(".player").classList.remove("rock","paper","scissors","lizard","spock");
    document.querySelector(".house").classList.remove("rock","paper","scissors","lizard","spock");
